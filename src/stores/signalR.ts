...
export const useSignalRStore = defineStore("signalR", () => {
  ...
  async function initWebSocket(accessToken: string) {
    ...
    startConnectionWithReconnect(
      emergencyHub.connection as signalR.HubConnection,
    ...
    // 連接 mqtt Hub
    startConnectionWithReconnect(
      mqttHub.connection as signalR.HubConnection,
      initMqttHubFunction
    );
    ...
    // 連接 fire Hub
    startConnectionWithReconnect(
      fireHub.connection as signalR.HubConnection,
      initFireHubFunction
    );
    ...
    // 連接 notify Hub
    startConnectionWithReconnect(
      notifyHub.connection as signalR.HubConnection,
      initNotifyHubFunction
    );
  }
  // 初始化 Hub
  function initConnection(Hub: Hub, hubName: string, accessToken: string) {
    const hubUrl = `${process.env.API}/hub/${hubName}`;

    Hub.connection = new signalR.HubConnectionBuilder()
      .withUrl(hubUrl, {
        accessTokenFactory: () => accessToken as string,
      })
      .configureLogging(signalR.LogLevel.Debug) // 設置日誌紀錄級別 Error (僅記錄錯誤訊息)、Warning (記錄警告跟錯誤訊息)、Information (不包含錯誤的狀態消息)、Trace (記錄所有訊息)、Debug (severity 最高，記錄所有訊息)
      .withAutomaticReconnect() // 自動重新連線
      .build();
    console.log(hubName, Hub.connection);

    Hub.connection.onclose((err) => {
      console.log(hubName + " 連接已經斷開 => 執行函数 onclose", err);
      Notify.create({
        type: "negative",
        message: hubName + " 連接已經斷開，建議重整頁面來重新連接",
        position: "top",
      });
    });
    Hub.connection.onreconnecting((error) => {
      console.assert(
        Hub.connection?.state === signalR.HubConnectionState.Reconnecting
      );
    ...
    Hub.connection.onreconnected((connectionId) => {
      console.assert(
        Hub.connection?.state === signalR.HubConnectionState.Connected
      );
      console.log(
  ...
  // 連接 Hub 方法
  async function startConnectionWithReconnect(
    connection: signalR.HubConnection,
    cb: () => void
  ) {
    try {
      await startConnection(connection, cb);
    } catch (err) {
      console.assert(
        ...
        if (isCenter.value || marshallingName.value.includes(task.role.name)) {
          if (
            (isCenter.value &&
              ...
              !task.needSupport &&
              !task.receiveUser) ||
            task.receiveUser?.id === userData.value.id
          ) {
            return;
          }
  ...
  function initNotifyHubFunction() {
    notifyHub.connection?.on("ReceiveMessage", (model: NotifyReadViewModel) => {
      ...
      if (userData.value && model.user.id === userData.value.id) {
        notifyWsData.value = model;
      }
    });
  }
